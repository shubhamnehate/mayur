...
def _get_amount(payload_amount: Optional[float], course_price: Decimal) -> Decimal:
    ...
    return Decimal(str(payload_amount)).quantize(Decimal("0.01"))
...
def create_order():
    ...
    try:
        ...
        return jsonify({"message": "course_id and user_id must be numeric"}), 400
...
def verify_payment():
    ...
    if not (order_id and payment_id and signature):
        return (
            jsonify({"message": "razorpay_order_id, razorpay_payment_id and razorpay_signature are required."}),
            400,
        )
